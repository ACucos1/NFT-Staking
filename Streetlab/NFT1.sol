...
contract StreetlabGenesis is ERC721A, Ownable {
    ...
    address internal constant _crossMintAddress =
    ...
    function _mintPayable(address to, uint256 amount) internal {
        ...
        require(
            supply + amount <= maxSupply - giveaway,
    ...
    function mintPresaleWithGiveaway(
        ...
        if (amountPresale > 0) {
            ...
            require(
                amountPresale <= maxTokenPresale,
                "Cannot mint more than maxTokens."
            );
            require(
                !_presaleNonces[noncePresale],
            ...
            uint256 supply = totalSupply();
            require(
                supply + amountPresale <= maxSupply - giveaway,
                "Not enough Tokens left."
            );
            require(
                amountPresale * price <= msg.value,
    ...
    function mintWithGiveaway(
        ...
        if (amountPublic > 0) {
            require(
                amountPublic <= maxTokensPerMint,
                "maxTokensPerMint exceeded!"
            );
            uint256 supply = totalSupply();
            require(
                supply + amountPublic <= maxSupply - giveaway,
                "Not enough Tokens left."
            );
            require(
                amountPublic * price <= msg.value,
                "Inconsistent amount sent!"
            );
        }
        if (amountGiveaway > 0) {
    ...
    //Crossmint is a platform which allows you to buy nfts with your credit card
    //_crossmintAddress is the address of the crossmint protocol
    ...
    function claimGiveaway(uint256 amount, address receiver)
        external
        onlyOwner
    {
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint256(blockhash(blockRef)) % maxSupply;
